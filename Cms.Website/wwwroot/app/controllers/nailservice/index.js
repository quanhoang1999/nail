var NailServiceController = function () {
        ...
        $('body').on('click', '.btn-delete', function (e) {
            ...
            system.confirm('Are you sure to delete?', function () {
                $.ajax({
                    ...
                    success: function () {
                        system.notify('Delete service successful', 'success');
                        system.stopLoading();
                        loadData();
                    },
                    error: function () {
    ...
    function loadData(isPageChanged) {
        $.ajax({
            type: "POST",
            url: "/admin/nailservice/filter",
            data: {
                Keyword: $('#txtKeyword').val(),
                CategoryId: $("#slSearchType").val(),
                ...
                if (response.RowCount > 0) {
                    $.each(response.Results, function (i, item) {
                        render += Mustache.render(template, {
                            Image: item.DefaultImage === undefined ? '<img src="/admin-side/images/user.png" width=25 />' : '<img src="' + item.DefaultImage + '" width=25 />',
                            CreatedDate: system.dateTimeFormatJson(item.DateCreated),
                            Category: item.NailCategory.Name,
