...
namespace Cms.Website.Areas.Admin.Controllers
{
    public class BillController : BaseController
    {
        ...
        public virtual IActionResult BillCreate(int id)
        {
            if (id > 0)
            {
                var model = _billService.GetByDetail(id);
                return View(model);
            }
            return View();
        }
        ...
        {
            var model = _billService.Filter(viewModel);
            return new OkObjectResult(model);
        }
        [HttpPost]
        ...
        //            var billDetail = _billService.GetById(billId);
