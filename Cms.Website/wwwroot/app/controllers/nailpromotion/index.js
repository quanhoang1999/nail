var NailPromotionController = function () {
    ...
    function registerEvents() {
        ...
        $('body').on('click', '.btn-delete', function (e) {
            ...
            system.confirm('Are you sure to delete?', function () {
                $.ajax({
                    type: "POST",
                    url: "/Admin/nailpromotion/Delete",
                    data: { id: that },
                    dataType: "json",
                    beforeSend: function () {
                        system.startLoading();
                    ...
                    success: function () {
                        system.notify('Delete promotion successful', 'success');
                        loadData();
                        system.stopLoading();
    ...
    function loadData(isPageChanged) {
        $.ajax({
            type: "POST",
            url: "/admin/nailpromotion/filter",
            data: {
                ...
                if (response.RowCount > 0) {
                    $.each(response.Results, function (i, item) {
                        render += Mustache.render(template, {
                            ...
                            IsShowHomePage: item.IsShowHomePage ? '<i class="fas fa-toggle-on"></i>' : '<i class="fas fa-toggle-off"></i>',
                            Id: item.Id,
                            StoreName: item.NailStoreName,
                            IsActive: item.IsActive ? '<i class="fas fa-toggle-on"></i>' : '<i class="fas fa-toggle-off"></i>',
                            Image: item.Image === undefined ? '<img src="/admin-side/images/user.png" height=50 />' : '<img src="' + item.ImageFullUrl + '" height=25 />'
                        });
                    });
